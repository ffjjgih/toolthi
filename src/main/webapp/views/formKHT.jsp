					...
					<ul class="container_button">
						<li>
							<div class="container_search">
								<i class="container_search-icon fas fa-search"></i> <input
									type="text" placeholder="VD IT16308" name="txtSearch"
						...
						<tbody>
							<c:forEach items="${lst}" var="item">
								<tr>
									...
									<td>${item.lop }</td>
									<td><a type="submit"
										href="updatekht?index=${item.id}&&id=${idkihoc}"
										class="button_update">SỬA</a> 
										<a type="submit" href="#"
										class="button_del" onclick="showMessage(${item.id})">XÓA</a></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
	...
	<script>
        ...
        function checkForm() {
         ...
         function showMessage(id){
            ...
            if(option === true){
                window.location.href = 'deletekht?khtID='+id+'&&ID=${idkihoc}';
            } else {
            }
        }
    </script>
